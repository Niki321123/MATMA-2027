...
window.cat02 = (() => {
  ...
  // === SCHEMAT E: granica ciągu rekurencyjnego ===
  // Wzorzec maturalny 8/10: a_{n+1} = f(a_n), zakładamy zbieżność, wyznaczamy granicę
  // Wymaga: przejścia granicą w obu stronach, rozwiązania równania algebraicznego na g
  function recursiveLimit() {
    const templates = [
      {
        formula: 'a_{n+1} = \\dfrac{a_n + \\dfrac{2}{a_n}}{2}',
        desc: 'metoda Herona dla \\sqrt{2}',
        limit: '\\sqrt{2}',
        steps: [
          { content: '\\text{Zakładamy, że } \\lim_{n\\to\\infty} a_n = g > 0.', explanation: '' },
          { content: 'g = \\dfrac{g + \\frac{2}{g}}{2}', explanation: 'Granica lewej = granica prawej strony wzoru.' },
          { content: '2g = g + \\frac{2}{g} \\implies g = \\frac{2}{g} \\implies g^2 = 2 \\implies g = \\sqrt{2}', explanation: '$g > 0$, więc bierzemy pierwiastek dodatni.' }
        ]
      },
      {
        formula: 'a_{n+1} = \\dfrac{1}{2}\\!\\left(a_n + \\dfrac{4}{a_n}\\right)',
        desc: 'metoda Herona dla \\sqrt{4}=2',
        limit: '2',
        steps: [
          { content: '\\text{Zakładamy, że } g = \\lim_{n\\to\\infty} a_n.', explanation: '' },
          { content: 'g = \\dfrac{1}{2}\\left(g + \\dfrac{4}{g}\\right)', explanation: '' },
          { content: '2g = g + \\dfrac{4}{g} \\implies g = \\dfrac{4}{g} \\implies g^2 = 4 \\implies g = 2', explanation: '$g > 0$.' }
        ]
      },
      {
        formula: 'a_{n+1} = \\sqrt{6 + a_n}',
        desc: 'ciąg zagnieżdżony',
        limit: '3',
        steps: [
          { content: '\\text{Zakładamy, że } g = \\lim_{n\\to\\infty} a_n \\geq 0.', explanation: '' },
          { content: 'g = \\sqrt{6 + g}', explanation: 'Funkcja $\\sqrt{6+x}$ jest ciągła, więc granicę można wstawić pod pierwiastek.' },
          { content: 'g^2 = 6 + g \\implies g^2 - g - 6 = 0 \\implies (g-3)(g+2) = 0 \\implies g = 3', explanation: '$g \\geq 0$, więc $g = -2$ odpada.' }
        ]
      },
      {
        formula: 'a_{n+1} = \\sqrt{12 + a_n}',
        desc: 'ciąg zagnieżdżony',
        limit: '4',
        steps: [
          { content: '\\text{Zakładamy, że } g = \\lim_{n\\to\\infty} a_n \\geq 0.', explanation: '' },
          { content: 'g = \\sqrt{12 + g}', explanation: '' },
          { content: 'g^2 - g - 12 = 0 \\implies (g-4)(g+3) = 0 \\implies g = 4', explanation: '$g \\geq 0$.' }
        ]
      },
      {
        formula: 'a_{n+1} = \\dfrac{a_n}{2} + \\dfrac{2}{a_n}',
        desc: 'iteracja Newtona dla \\sqrt{4}',
        limit: '2',
        steps: [
          { content: '\\text{Zakładamy, że } g = \\lim_{n\\to\\infty} a_n > 0.', explanation: '' },
          { content: 'g = \\dfrac{g}{2} + \\dfrac{2}{g}', explanation: '' },
          { content: '\\dfrac{g}{2} = \\dfrac{2}{g} \\implies g^2 = 4 \\implies g = 2', explanation: '$g > 0$.' }
        ]
      },
      {
        formula: 'a_{n+1} = \\dfrac{3a_n + 5}{a_n + 3}',
        desc: 'ciąg ułamkowy',
        limit: '\\sqrt{5}',
        steps: [
          { content: 'g = \\lim_{n\\to\\infty} a_n.', explanation: 'Zakładamy zbieżność.' },
          { content: 'g = \\dfrac{3g + 5}{g + 3}', explanation: '' },
          { content: 'g(g+3) = 3g+5 \\implies g^2+3g = 3g+5 \\implies g^2 = 5 \\implies g = \\sqrt{5}', explanation: '$g > 0$.' }
        ]
      },
    ];
    ...
    return {
      id: M.makeId('cat02_recursive'),
      category: 2,
      categoryName: 'Granica',
      type: 'recursive_limit',
      points: 4,
      params: { limit: tpl.limit },
      statement:
        `Dany jest ciąg $(a_n)$ określony wzorem rekurencyjnym\n$$${tpl.formula}$$\n` +
        `Zakładając, że ciąg jest zbieżny, wyznacz jego granicę. Zapisz obliczenia.`,
      answer: {
        type: 'expression',
        value: tpl.limit,
        display: tpl.limit,
        description: `$\\lim_{n\\to\\infty} a_n = ${tpl.limit}$`
      },
      hints: [
        { level: 1, text: `Zakładamy $\\lim_{n\\to\\infty} a_n = g$. Przechodzimy do granicy po obu stronach wzoru rekurencyjnego: lewa strona $\\to g$, prawa strona $\\to$ wyrażenie zależne od $g$.` },
        { level: 2, text: `Otrzymujesz równanie algebraiczne na $g$. Rozwiąż je.` },
        { level: 3, text: `Bierz rozwiązanie zgodne z zachowaniem ciągu (tu: $g > 0$ lub $g \\geq 0$). Odpowiedź: $g = ${tpl.limit}$.` }
      ],
      solution: tpl.steps.map((s, i) => ({
        step: i + 1,
        title: i === 0 ? 'Założenie zbieżności' : i === 1 ? 'Równanie na granicę' : 'Wyznaczenie $g$',
        content: s.content,
  ...
  function generate() {
    // recursiveLimit (8/10) i sqrtLimit (7/10) — główne schematy maturalne
    // funcLimit (6/10) zostawiony jako fallback dla różnorodności
    return M.choose([recursiveLimit, sqrtLimit, recursiveLimit, sqrtLimit, recursiveLimit, funcLimit])();
  }
