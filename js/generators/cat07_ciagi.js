...
window.cat07 = (() => {
  ...
  // === SCHEMAT E: Z sumy S_n odtwórz wyraz ogólny a_n ===
  // Wzorzec inspirowany: a_n = S_n - S_{n-1} dla n ≥ 2; a_1 = S_1 — pułapka!
  // Generator: S_n = A·n² + B·n   (C=0 jest WYMAGANE bo S_0 = 0)
  // a_n = A(2n-1) + B = 2A·n + (B - A)   →  ciąg arytmetyczny z r = 2A
  function sumToTerm() {
    for (let attempt = 0; attempt < 20; attempt++) {
      const A = M.choose([1, 2, 3, 4]);
      const B = M.rand(-8, 8);
      // S_n = A·n² + B·n
      // a_n = 2A·n + (B - A) dla n ≥ 1
      const r = 2 * A;
      const a1 = A + B;
      // Quality gates
      if (Math.abs(r) < 2) continue;
      if (Math.abs(a1) > 20) continue;
      // Wybierz konkretne k do obliczenia
      const k = M.choose([10, 15, 20, 25]);
      const ak = r * k + (B - A);
      const Sk = A * k * k + B * k;

      return {
        id: M.makeId('cat07_sum_to_term'),
        category: 7, categoryName: 'Ciągi liczbowe',
        type: 'sum_to_general_term', points: 4,
        params: { A, B, r, a1, k, ak, Sk },
        statement:
          `Suma $n$ początkowych wyrazów pewnego ciągu liczbowego $(a_n)$, określonego dla każdej liczby naturalnej $n \\geq 1$, dana jest wzorem\n` +
          `$$S_n = ${A === 1 ? '' : A}n^2 ${B >= 0 ? '+' + B : B}n$$\n\n` +
          `**a)** Wyznacz wzór ogólny ciągu $(a_n)$.\n\n` +
          `**b)** Wykaż, że ciąg $(a_n)$ jest arytmetyczny i wyznacz jego pierwszy wyraz oraz różnicę.\n\n` +
          `**c)** Oblicz $a_{${k}}$. Zapisz obliczenia.`,
        answer: {
          type: 'multipart',
          display: `a_n = ${r}n${(B - A) >= 0 ? '+' + (B - A) : (B - A)},\\quad a_1 = ${a1},\\quad r = ${r},\\quad a_{${k}} = ${ak}`,
          description: `$a_n = ${r}n${(B - A) >= 0 ? '+' + (B - A) : (B - A)}$, $a_1 = ${a1}$, $r = ${r}$, $a_{${k}} = ${ak}$`,
        },
        hints: [
          { level: 1, text: 'Dla $n \\geq 2$: $a_n = S_n - S_{n-1}$. Osobno sprawdź $a_1 = S_1$.' },
          { level: 2, text: `$S_{n-1} = ${A}(n-1)^2 ${B >= 0 ? '+' + B : B}(n-1)$. Odejmij od $S_n$.` },
          { level: 3, text: `Otrzymasz $a_n = ${r}n ${(B - A) >= 0 ? '+' + (B - A) : (B - A)}$. Sprawdź $a_1 = ${r} ${(B - A) >= 0 ? '+' + (B - A) : (B - A)} = ${a1}$ ✓ (zgadza się z $S_1$).` },
        ],
        solution: [
          { step: 1, title: 'Wzór na $a_n$ dla $n \\geq 2$', content: `a_n = S_n - S_{n-1}`, explanation: 'Definicja: suma $n$ wyrazów minus suma $(n-1)$ wyrazów.' },
          { step: 2, title: 'Podstawienie', content: `a_n = \\bigl(${A === 1 ? '' : A}n^2 ${B >= 0 ? '+' + B : B}n\\bigr) - \\bigl(${A === 1 ? '' : A}(n-1)^2 ${B >= 0 ? '+' + B : B}(n-1)\\bigr)`, explanation: '' },
          { step: 3, title: 'Uproszczenie', content: `a_n = ${A}\\bigl(n^2 - (n-1)^2\\bigr) ${B >= 0 ? '+' + B : B}\\bigl(n - (n-1)\\bigr) = ${A}(2n-1) ${B >= 0 ? '+' + B : B} = ${r}n ${(B - A) >= 0 ? '+' + (B - A) : (B - A)}`, explanation: '' },
          { step: 4, title: 'Sprawdzenie $a_1$ (uwaga: pułapka!)', content: `a_1 \\stackrel{?}{=} S_1 = ${A} ${B >= 0 ? '+' + B : B} = ${a1}\\\\ \\text{Ze wzoru: }a_1 = ${r}\\cdot 1 ${(B - A) >= 0 ? '+' + (B - A) : (B - A)} = ${a1}\\quad\\checkmark`, explanation: 'Wzór $a_n = S_n-S_{n-1}$ działa od $n=2$, ale w tym przypadku zgadza się też dla $n=1$.' },
          { step: 5, title: 'Wykazanie arytmetyczności', content: `a_{n+1} - a_n = \\bigl(${r}(n+1) ${(B - A) >= 0 ? '+' + (B - A) : (B - A)}\\bigr) - \\bigl(${r}n ${(B - A) >= 0 ? '+' + (B - A) : (B - A)}\\bigr) = ${r}`, explanation: `Różnica między kolejnymi wyrazami stała → ciąg arytmetyczny, $r = ${r}$.` },
          { step: 6, title: `Obliczenie $a_{${k}}$`, content: `a_{${k}} = ${r}\\cdot${k} ${(B - A) >= 0 ? '+' + (B - A) : (B - A)} = ${ak}`, explanation: '' },
        ],
      };
    }
    return null;
  ...
  function generate() {
    const variants = [arithGeomCombo, geomInfinite, geomTwoConditions,
                      arithGeomCross, loanRepayment, geometricProductionGrowth,
                      sumToTerm, sumToTerm];  // sumToTerm dwukrotnie — wzmocnienie wzorca CKE
    for (let i = 0; i < 5; i++) {
      const v = M.choose(variants);
      const result = v();
      if (result) return result;
    }
    return arithGeomCombo();
  }

  return { generate, sumToTerm };
})();
